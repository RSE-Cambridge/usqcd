...
static void point_source(complex *src, int x0, int y0, int z0, int t0)
{
  ...
  /* Here x0 is actually ~px, etc. */
  double px = 2*PI*x0/nx;
  double py = 2*PI*y0/ny;
  double pz = 2*PI*z0/nz;
  double pt = 2*PI*t0/nt;
  double p_dot_x;
  ...
  FORALLSITES(i,s){
    p_dot_x = px*(s->x) + py*(s->y) + pz*(s->z) + pt*(s->t);
    src[i].real = cos(p_dot_x);
    src[i].imag = sin(p_dot_x);
  }
  /*
...
static void random_color_wall(su3_vector *v, int t0){
  ...
  FORALLSITES(i,s){
	...
	v[i].c[jc].imag = gaussian_rand_no(&(s->site_prn));
        // Optional - U(1) normalisation instead of colour vector normalised
	Norm = v[i].c[jc].real*v[i].c[jc].real
	  + v[i].c[jc].imag * v[i].c[jc].imag;// RJD
	v[i].c[jc].real =  v[i].c[jc].real / sqrt( Norm );// RJD
	v[i].c[jc].imag =  v[i].c[jc].imag / sqrt( Norm );// RJD
      }
    }
  }
}
